...
export async function generateChatResponse(userMessage, reading, cards) {
  try {
    ...
    const response = await callOpenAIApi(userMessage, reading, cards)
    return response
  } catch (error) {
...
// 调用OpenAI API的示例函数
async function callOpenAIApi(userMessage, reading, cards) {
  // 构建请求体，包含用户问题和上下文信息
  const requestBody = buildApiRequestBody(userMessage, reading, cards)
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    headers: {
      ...
      'Authorization': `Bearer ${process.env.VITE_OPENAI_API_KEY}`, // 使用环境变量存储API密钥
    },
...
// 构建API请求体
function buildApiRequestBody(userMessage, reading, cards) {
  // 准备上下文信息
  const context = `用户刚刚完成了一次塔罗牌解读，抽取了三张牌：
  - 过去: ${cards[0].name}（${cards[0].meaning}）
  - 现在: ${cards[1].name}（${cards[1].meaning}）
  - 未来: ${cards[2].name}（${cards[2].meaning}）
  
AI解读师给出的解读内容如下：
  - 过去: ${reading.past}
  - 现在: ${reading.present}
  - 未来: ${reading.future}
  - 总结: ${reading.conclusion}
  
请根据这些上下文信息，以塔罗牌解读师的身份，回答用户的问题，提供深入、个性化的解释和建议。`
  ...
  return {
    ...
    messages: [
      {
        role: 'system',
        content: '你是一位经验丰富的塔罗牌解读师，可以根据用户的问题和当前的牌阵解读，提供深入、有洞察力的回答。请保持专业、温暖和支持的语气。'
      },
      {
        role: 'user',
        content: `${context}\n\n用户的问题：${userMessage}`
...
function getFallbackResponse(userMessage) {
  const responses = [
    ...
    '当前AI服务繁忙，请稍等片刻再提问。',
    '网络连接不稳定，无法获取AI回复。请检查您的网络连接或稍后再试。'
  ]
  
  return responses[Math.floor(Math.random() * responses.length)]
}

// 为了方便开发和测试，可以保留原有的模拟回复功能
